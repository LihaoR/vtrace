...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 19 23:41:31 2019
    ...
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope != 'global':
                ...
                self.value_loss = 0.5 * tf.reduce_mean(tf.square(self.v_target - self.values))
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,bootstrap_value):
        ...
        values            = rollout[:,5]
        bootstrap_value_ = bootstrap_value

        v_target1 = []
        delta_v = []
        for i in range(len(values)-1):
            delta_v.append(rewards[i] + gamma * values[i+1] - values[i])
        delta_v.append(rewards[-1] + gamma * bootstrap_value_ - values[-1])
        
        vs = values[-1] + delta_v[-1]
        v_target1.append(vs)
        delta_v.pop()
        for i in range(len(delta_v))[::-1]:
            vs = gamma * (vs - values[i+1]) + delta_v[i] + values[i]
            v_target1.append(vs)
        v_target1.reverse()
        
        advantages = gamma * np.array(v_target1[1:]) - np.array(values[:10]) + np.array(rewards[:10])
        v_target = v_target1[:10]

        feed_dict = {self.local_AC.v_target:v_target,
                     self.local_AC.inputs:np.vstack(observations[:10]),
                     self.local_AC.actions:actions[:10],
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if len(episode_buffer) == 11 and d != True:
